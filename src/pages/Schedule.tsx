...
const Schedule = () => {
  ...
  const [isAddingClassTypeStandalone, setIsAddingClassTypeStandalone] = useState(false);
  ...
  return (
    <Layout>
                    ...
                    <CardContent className="p-0">
                      <div className="flex items-stretch">
                        ...
                        <div className="flex-1 p-3">
                          ...
                          <button
                              onClick={(e) => { e.stopPropagation(); setViewingClassMode("general"); setViewingClass(cls); }}
      ...
      {/* Standalone Class Type Add Dialog */}
      <Dialog open={isAddingClassTypeStandalone} onOpenChange={setIsAddingClassTypeStandalone}>
        <DialogContent className="max-w-md p-0 overflow-hidden [&>button]:hidden" dir="rtl">
          <motion.div
              ...
              <div className="flex gap-2 pt-2">
                <Button onClick={async () => {
                  ...
                  const { error } = await supabase.from("classes").insert({
                    name: newClassType.name,
                    description: newClassType.description,
                    image_url: newClassType.image_url || null,
                    image_position: newClassType.image_position,
                    level: newClassType.level,
                    day: "_type",
                    time: "00:00",
                  ...
                  if (error) { toast.error("שגיאה: " + error.message); }
                  else { toast.success("סוג השיעור נוסף"); queryClient.invalidateQueries({ queryKey: ["classes"] }); }
                  setNewClassType({ name: "", description: "", image_url: null, image_position: "50% 50%", level: "all" });
                  setIsAddingClassTypeStandalone(false);
                }} className="rounded-full flex-1 gap-2">
                  <Check className="h-4 w-4" />שמור סוג שיעור
                </Button>
                <Button variant="outline" onClick={() => setIsAddingClassTypeStandalone(false)} className="rounded-full">ביטול</Button>
              </div>
            </div>
          </motion.div>
        </DialogContent>
        ...
        {allClasses && !isNew && (
          <FormSection icon={BookOpen} title="סוג השיעור">
            <Popover open={showTypePicker} onOpenChange={setShowTypePicker}>
              ...
              <PopoverContent className="w-72 p-2 max-h-[50vh] overflow-y-auto overscroll-contain" align="start" dir="rtl" style={{ pointerEvents: 'auto' }}>
            ...
            <Popover open={showTeacherPicker} onOpenChange={setShowTeacherPicker}>
              ...
              <PopoverContent className="w-64 p-2 max-h-[50vh] overflow-y-auto overscroll-contain" align="start" dir="rtl" style={{ pointerEvents: 'auto' }}>
                ...
                <div className="border-t border-border/30 mt-1 pt-1">
                  <button
                    onClick={() => {
                      onChange({ ...value, teacher: "מורה מתחלף" });
                      setShowTeacherPicker(false);
                    }}
                    className={cn(
                      "w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-right transition-colors",
                      value.teacher === "מורה מתחלף" ? "bg-primary/10 text-primary" : "hover:bg-muted"
                    )}
                  >
                    ...
                    <div className="flex flex-col items-start">
                      <span className="text-sm font-medium">מורה מתחלף</span>
                      <span className="text-[10px] text-muted-foreground">מקשר לדף הצוות</span>
                    </div>
                    {value.teacher === "מורה מתחלף" && <Check className="h-4 w-4 text-primary mr-auto" />}
                  </button>
                  <button
                    onClick={() => {
